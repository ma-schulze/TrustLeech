...
# endif /* !PRELOADED_BL33_BASE */
	  ...
	  .next_handoff_image_id = TRUSTLEECH_IMAGE_ID,
	},

	{ .image_id = TRUSTLEECH_IMAGE_ID,
	  SET_STATIC_PARAM_HEAD(ep_info, PARAM_EP, VERSION_2,
				entry_point_info_t, NON_SECURE | EXECUTABLE),
	  .ep_info.pc = TRUSTLEECH_ENTRYPOINT,
	  .ep_info.spsr = SPSR_64(MODE_EL2, MODE_SP_ELX, DISABLE_ALL_EXCEPTIONS),
	  SET_STATIC_PARAM_HEAD(image_info, PARAM_EP, VERSION_2,
				image_info_t, 0),
	  .image_info.image_base = TRUSTLEECH_BASE,
	  .image_info.image_max_size = TRUSTLEECH_AREA_SIZE,
	  .next_handoff_image_id = INVALID_IMAGE_ID,
	}
