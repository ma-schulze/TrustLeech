...
static addr_t
parse_mtree_aarch64(char *mtree_output)
{
    char *ptr = NULL;
    char *tmp = NULL;
    char *line = NULL;
    ...
    addr_t *maximum_physical_address)
{
    ...
    *allocated_ram_size = info.maxMem * 1024; // convert KBytes to bytes
    ...
    addr_t parsed_max = parse_mtree_aarch64(bufstr);
    ...
    unsigned long vcpu)
{
    ...
    switch (reg) {
        ...
        // aarch64 registers 
        case CPSR:
            *value = parse_reg_value("PSTATE", regs);
            break;
        case TTBCR:
            *value = parse_reg_value("TCR_EL1", regs);
            break;
        case TTBR0:
            *value = parse_reg_value("TTBR0_EL1", regs);
            break;
        case TTBR1:
            *value = parse_reg_value("TTBR1_EL1", regs);
            break;
        case SCTLR:
            *value = parse_reg_value("SCTLR_EL1", regs);
            break;
        default:
