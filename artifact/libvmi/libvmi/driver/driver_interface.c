...
#ifdef ENABLE_TRUSTLEECH
#include "driver/trustleech/trustleech.h"
#endif
...
#ifdef ENABLE_TRUSTLEECH
    if (VMI_SUCCESS == trustleech_test(domainid, name, init_flags, init_data)) {
        dbprint(VMI_DEBUG_DRIVER, "--found TrustLeech\n");
        *mode = VMI_TRUSTLEECH;
        count++;
    }
#endif
...
#ifdef ENABLE_TRUSTLEECH
        case VMI_TRUSTLEECH:
            rc = driver_trustleech_setup(vmi);
            break;
#endif
        default:
            break;
