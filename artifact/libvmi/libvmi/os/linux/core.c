...
static status_t linux_filemode_init(vmi_instance_t vmi)
{
    ...
    switch (vmi->page_mode) {
        case VMI_PM_AARCH64:
            linux_symbol_to_address(vmi, "_text", NULL, &virt_start);
    ...
    if (VMI_SUCCESS == rc && !(vmi->page_mode == VMI_PM_AARCH64)) {
    ...
    addr_t test = 0;
    if(vmi->page_mode == VMI_PM_AARCH64) {
        // printf("Trying everything\n");
        rc = vmi_get_vcpureg(vmi, &kernel_pgt, TTBR1, 0);
        kernel_pgt &= 0xfffffffffffe;
        vmi->kpgd = kernel_pgt;

        rc = vmi_pagetable_lookup(vmi, kernel_pgt, virt_start, &test);
        if ( VMI_SUCCESS == rc) {
          // printf("lelellele\n");
