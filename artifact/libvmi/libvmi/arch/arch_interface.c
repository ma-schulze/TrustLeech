...
static status_t get_vcpu_page_mode_arm(vmi_instance_t vmi, unsigned long vcpu, page_mode_t *out_pm)
{
    ...
    dbprint(VMI_DEBUG_PTLOOKUP, "getting arm page mode\n");
    ...
    dbprint(VMI_DEBUG_PTLOOKUP, "cpsr 0x%x\n", cpsr);
...
{
    dbprint(VMI_DEBUG_PTLOOKUP, "Getting PM0\n");
    if (VMI_FILE == vmi->mode) {
        /* skip all of this for files */
        return VMI_FAILURE;
    }
    dbprint(VMI_DEBUG_PTLOOKUP, "Getting PM1\n");
